...
export default class DepositUpdate extends React.Component {
    constructor (props){
        super(props);
        this.state ={
            type: '',
    ...
    handleSubmit(event) {
        ...
        Api.put(`/deposits/${this.props.match.params.id}`, deposit).then( () => {
            this.setState({message: "Deposit updated successfully!"});
        }).catch( (err) => {
    ...
    componentDidMount() {
        Api.get(`/deposits/${this.props.match.params.id}`).then( res => {
    ...
    render(){
        return (
            <Card className="mt-4 mx-auto text-center" style={{maxWidth:"500px"}}>
                <h5 className="card-header">Please update your deposit!</h5>
                <div className="card-body">
                    <form onSubmit={this.handleSubmit}>
                        ...
                        <div className=" text-center">
                            <button className=" btn btn-primary mr-1" type={"submit"} value={"Submit"}>Update Deposit</button>
